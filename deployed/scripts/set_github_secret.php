...
$res = curl_exec($ch);
if ($res === false) { fwrite(STDERR, "Failed to get public key: " . curl_error($ch) . "\n"); exit(4); }
$code = curl_getinfo($ch, CURLINFO_HTTP_CODE);
curl_close($ch);
if ($code !== 200) { fwrite(STDERR, "GitHub API returned HTTP $code when fetching public key\n"); fwrite(STDERR, $res . "\n"); exit(5); }
$obj = json_decode($res, true);
$key = $obj['key'] ?? null;
$keyId = $obj['key_id'] ?? ($obj['id'] ?? null);
if (!$key || !$keyId) { fwrite(STDERR, "Invalid public key response\n"); exit(6); }
...
$res2 = curl_exec($ch);
if ($res2 === false) { fwrite(STDERR, "Failed to PUT secret: " . curl_error($ch) . "\n"); exit(8); }
