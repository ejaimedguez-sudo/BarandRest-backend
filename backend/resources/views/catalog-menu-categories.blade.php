...
<head>
    ...
    <title>Catalogo de Categorias de Menu - Ordena Facil</title>
...
<section id="editorFrame" class="editor-frame" aria-hidden="true">
    <div class="editor-head">
        <h2 id="formTitle">Nueva categoria de menu</h2>
        <button id="btnCloseEditor" class="btn btn-compact" type="button">Cerrar</button>
    </div>
    <div class="editor-body">
        <form id="categoryForm" class="form-grid cols-2">
            <input id="categoryId" type="hidden">

            <div class="field">
                <label for="code">Codigo</label>
                <input id="code" name="code" type="text" maxlength="80" pattern="[A-Za-z0-9\-_.]+" title="Solo letras, numeros, guion, guion bajo y punto" placeholder="CAT-COCTEL">
            </div>

            <div class="field">
                <label for="name" class="required">Nombre</label>
                <input id="name" name="name" type="text" maxlength="120" required placeholder="Coctel, Entrada, Postre...">
            </div>

            <div class="field">
                <label for="sort_order">Orden</label>
                <input id="sort_order" name="sort_order" type="number" min="0" step="1" placeholder="10">
            </div>

            <div class="field field-wide">
                <label for="description">Descripcion</label>
                <textarea id="description" name="description" maxlength="1000" placeholder="Describe el tipo de categoria"></textarea>
            </div>
...
<script>
    ...
    function getSelectedCategory() {
        return categories.find((item) => Number(item.id) === Number(selectedCategoryId)) || null;
    }

    function updateActionButtons() {
        const hasSelection = !!getSelectedCategory();
    ...
    function collectPayload() {
        return {
            ...
            sort_order: normalizeNumber(fields.sort_order.value),
    ...
    function openEditor(mode) {
        ...
        if (mode === 'edit') {
            ...
            fields.code.value = category.code || '';
    ...
    function renderTable(rows) {
        if (!rows.length) {
            tableContainer.innerHTML = '<div class="empty">No hay categorias de menu registradas.</div>';
        ...
        const body = rows.map((category) => {
            const selected = Number(selectedCategoryId) === Number(category.id) ? ' class="selected"' : '';
            return `
                <tr data-category-id="${category.id}"${selected}>
    ...
    function applyFilter() {
        ...
        const filtered = categories.filter((item) => {
            const line = `${item.code || ''} ${item.name || ''} ${item.description || ''}`.toLowerCase();
    ...
    async function loadCategories() {
        try {
            ...
            tableContainer.innerHTML = '<div class="empty">No fue posible cargar categorias de menu.</div>';
            setStatus(`Error cargando categorias: ${String(error.message || error)}`, 'error');
        }
    }
    ...
    async function removeSelectedCategory() {
        ...
        const label = category?.name ? `"${category.name}"` : `#${category.id}`;
        if (!window.confirm(`¿Deseas eliminar la categoria de menu ${label}? Esta accion no se puede deshacer.`)) return;

        try {
            await requestJson(`/api/menu-categories/${category.id}`, { method: 'DELETE' });
            selectedCategoryId = null;
    ...
    categoryForm.addEventListener('submit', async (event) => {
        ...
        try {
            if (editingId) {
                await requestJson(`/api/menu-categories/${editingId}`, {
                    method: 'PUT',
                ...
                await requestJson('/api/menu-categories', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload),
                });
    ...
    tableContainer.addEventListener('click', (event) => {
        ...
        selectedCategoryId = Number(row.dataset.categoryId);
        applyFilter();
